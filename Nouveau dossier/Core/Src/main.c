...
/* USER CODE BEGIN PV */
IKS01A3_MOTION_SENSOR_Axes_t gyro_data;
int32_t myThreshold = 500;
/* USER CODE END PV */
...
int divide_and_ceil(int v, int divisor) {
    return (v + divisor - 1) / divisor;
}


void format_msg(char* msg, int v,int Tunits, int Tdecimals,int Hunits, int Hdecimals,int Punits, int Pdecimals)
{
	sprintf(msg, "<V%04dT%02d%02dH%02d%02dP%04d%02d>",
			v, Tunits, Tdecimals,
			Hunits, Hdecimals,
			Punits, Pdecimals);
}
